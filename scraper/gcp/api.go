...
func determineMachineFamily(name string) string {
	...
	case strings.Contains(nameLower, "highcpu"),
		...
		strings.HasPrefix(nameLower, "c4n-"),
...
var machineTypeRegex = regexp.MustCompile(`(?i)(n1|n2d|n2|n4d|n4a|n4|e2|e2a|c2|c2d|m1|m2|m3|m4n|m4|x4|t2d|t2a|a2|a3|a4x|a4|g2|g4|h3|h4d|c3|c3d|z3|c4a|c4d|c4n|c4)\b.*(?:instance\s+(core|ram)|\((?:cpu|ram)\s+cost\))`)

// legacySKURegex matches the two first-generation SKU naming formats that
// predate the "<FAMILY> Instance Core/Ram" convention and carry no machine
// family token for machineTypeRegex to capture:
//
//	"Compute optimized Core running in Americas"          (C2)
...
var legacySKURegex = regexp.MustCompile(`(?i)\b(compute optimized|memory-optimized instance)\s+(core|ram)\b`)
...
var cudSKURegex = regexp.MustCompile(`(?i)^commitment\s+v\d+:\s+(n1|n2d|n2|n4d|n4a|n4|e2|e2a|c2|c2d|m1|m2|m3|m4n|m4|x4|t2d|t2a|a2|a3|a4x|a4|g2|g4|h3|h4d|c3|c3d|z3|c4a|c4d|c4n|c4)\s+(?:[a-z0-9]+\s+)*?(cpu|ram)\s+in\s+.+\s+for\s+(1|3)\s+year`)
	...
	if machineFamily == "" && !strings.Contains(strings.ToLower(displayName), "premium") {
		if legacyMatches := legacySKURegex.FindStringSubmatch(displayName); len(legacyMatches) >= 3 {
			...
			case "compute optimized":
				machineFamily = "C2"
			case "memory-optimized instance":
				machineFamily = "M1"
			}
